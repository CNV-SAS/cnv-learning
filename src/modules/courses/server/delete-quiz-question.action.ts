...
// Server action: eliminar pregunta de quiz (Bloque 23.2.b). Patron
// thin. CASCADE elimina las opciones automaticamente.

import { revalidatePath } from "next/cache";
import { profileRepository } from "@/modules/auth/data/profile.repository";
import { quizEditorService } from "@/modules/courses/services/quiz-editor.service";
import { deleteQuizQuestionSchema } from "@/modules/courses/validations";
...
export async function deleteQuizQuestionAction(
  ...
  try {
    return await withContext({ requestId }, async () => {
      ...
      const result = await quizEditorService.deleteQuestion({
        user,
    ...
    logger.error("deleteQuizQuestionAction unexpected throw", {
