...
import { deleteCourseResourceSchema } from "@/modules/courses/validations";
...
export async function deleteCourseResourceAction(
  ...
  try {
    return await withContext({ requestId }, async () => {
      const parsed = deleteCourseResourceSchema.safeParse(input);
      if (!parsed.success) {
        return err(
      ...
      const result = await courseResourceService.deleteResource({
        user,
    ...
    logger.error("deleteCourseResourceAction unexpected throw", {
