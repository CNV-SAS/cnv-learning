...
import { reorderLessonSchema } from "@/modules/courses/validations";
...
export async function reorderLessonAction(
  ...
  try {
    return await withContext({ requestId }, async () => {
      ...
      const result = await courseContentEditorService.reorderLesson({
        user,
    ...
    logger.error("reorderLessonAction unexpected throw", {
