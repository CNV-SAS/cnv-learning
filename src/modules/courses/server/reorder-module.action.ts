...
import { reorderModuleSchema } from "@/modules/courses/validations";
...
export async function reorderModuleAction(
  ...
  try {
    return await withContext({ requestId }, async () => {
      ...
      const result = await courseContentEditorService.reorderModule({
        user,
    ...
    logger.error("reorderModuleAction unexpected throw", {
